...
function registerAdminShiftSettings(bot, ensureUser, logError) {
  ...
  // src/bot/admin/shiftSettings.js
  const { Markup } = require("telegraf");
  const pool = require("../../db/pool"); // <-- ДОБАВЬ (нужно для выборок)
  const { deliver } = require("../../utils/renderHelpers");
  ...
  async function loadDayAssignments() {
    // Схема совпадает с тем, как задачи подтягиваются для “Задачи на сегодня”
    // (join assignments + schedules + templates) :contentReference[oaicite:7]{index=7}
  ...
  `,
    ...
    const type = `Тип ответа: <b>${esc(r.answer_type)}</b>`;
  ...
  function registerAdminShiftSettings(bot, ensureUser, logError) {
    // ... твой существующий admin_shift_settings entry оставляем как есть :contentReference[oaicite:8]{index=8}

    // ✅ РЕАЛЬНЫЙ ROOT РАЗДЕЛА "в течение дня"
